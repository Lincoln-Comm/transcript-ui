...
export function transformStudent(apiStudent: StudentAPI): Student {
  ...
  const formatGender = (g: string | null): string | null => {
    if (!g) return null;
    if (g === 'M') return 'Male';
  ...
  const nameParts = [apiStudent.first_name];
  if (apiStudent.middle_name) {
    nameParts.push(apiStudent.middle_name);
  }
  nameParts.push(apiStudent.last_name);
  ...
  return {
    ...
    grade,
    dateOfBirth: apiStudent.date_of_birth ? formatDate(apiStudent.date_of_birth) : null,
    gender: formatGender(apiStudent.gender),
    nationality: apiStudent.nationality || null,
    yearGroup: apiStudent.year_group,
    status: apiStudent.status,
    entryDate: apiStudent.entry_date ? formatDate(apiStudent.entry_date) : null,
    entryGrade: apiStudent.entry_grade || null,
    dateOfGraduation: apiStudent.graduation_date 
      ? formatDate(apiStudent.graduation_date) 
      : calculateGraduationDate(apiStudent.entry_grade || null, apiStudent.entry_date || null),
  };
}
