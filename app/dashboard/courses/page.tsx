    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          />
          <div 
            className="fixed w-40 bg-white rounded-xl shadow-xl border border-gray-100 py-1 z-50"
...
function ViewCourseModal({ 
  ...
  const fields = [
    ...
    { label: 'Grade Level', value: course.grade_level || '-' },
...
function CourseFormModal({ 
  ...
  const modalTitle = isEdit ? 'Edit Course' : isTemplate ? 'Create from Template' : 'Add New Course';
        ...
        {isTemplate && (
          <div className="px-6 py-3 bg-purple-50 border-b border-purple-100">
            <p className="text-sm text-purple-700">
              Creating a new course based on template. Modify the details below as needed.
            </p>
          </div>
        )}
...
function CoursesContent() {
  ...
  const [templateCourse, setTemplateCourse] = useState<CourseAPIResponse | null>(null);
  const [showAddDropdown, setShowAddDropdown] = useState(false);
  const [allCourses, setAllCourses] = useState<CourseAPIResponse[]>([]);
  ...
  // Filtered courses for template selection (uses allCourses, not paginated courses)
  const filteredTemplateCourses = allCourses.filter(course => {
    if (!templateSearch) return true;
    const search = templateSearch.toLowerCase();
    const nameMatch = course.course_name?.toLowerCase().includes(search);
    const descMatch = course.course_description?.toLowerCase().includes(search);
    const levelMatch = course.course_level?.toLowerCase().includes(search);
    const gradeLevelMatch = course.grade_level?.toLowerCase().includes(search);
    return nameMatch || descMatch || levelMatch || gradeLevelMatch;
  });

  // Fetch all courses for template selection
  const fetchAllCoursesForTemplates = async () => {
    setIsLoadingTemplates(true);
    try {
      // Fetch with a very high limit to get all courses
      const response = await getCoursesPaginated({
        page: 1,
        limit: 1000,
      });
      ...
      console.error('Error fetching courses for templates:', err);
    } finally {
  ...
  const handleOpenTemplateModal = () => {
    ...
    fetchAllCoursesForTemplates();
  };

  // Handle create from template
  const handleCreateFromTemplate = (course: CourseAPIResponse) => {
    setTemplateCourse(course);
  ...
  const handleSave = async (data: CourseFormData) => {
    ...
    try {
      const payload = {
        ...
        course_description: data.course_description || undefined,
        course_level: data.course_level,
        course_number: data.course_number ? parseInt(data.course_number) : undefined,
        course_alias: data.course_alias || undefined,
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      ...
                      <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wide">Grade Level</th>
                    ...
                    {courses.map((course) => (
                      <tr key={course.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-gray-600 max-w-xs">
                          <p className="truncate" title={course.course_description || ''}>
                        ...
                        <td className="px-6 py-4 text-gray-600">{course.grade_level || '-'}</td>
      ...
      {(showAddModal || editCourse) && (
        <CourseFormModal
          course={editCourse ? {
            ...
            course_name: templateCourse.course_name || '',
            course_description: templateCourse.course_description || '',
            course_level: templateCourse.course_level || '',
            course_number: '',
            course_alias: '',
            grade_level: templateCourse.grade_level || '',
          } : emptyFormData}
          isEdit={!!editCourse}
          isTemplate={!!templateCourse && !editCourse}
          onClose={() => { setShowAddModal(false); setEditCourse(null); setTemplateCourse(null); }}
          onSave={handleSave}
          isSaving={isSaving}
        />
      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Choose an existing course to use as a template</p>
                </div>
              </div>
              <button
                onClick={() => { setShowTemplateModal(false); setTemplateSearch(''); setAllCourses([]); }}
                className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
              >
                <X className="w-5 h-5 text-gray-500" />
              </button>
            ...
            <div className="px-6 py-4 border-b border-gray-100">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by name, description, or level..."
            ...
            <div className="p-4 overflow-y-auto max-h-[calc(90vh-220px)]">
              ...
              ) : filteredTemplateCourses.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-gray-500">
                  <BookOpen className="w-12 h-12 mb-4 text-gray-300" />
                  <p>{templateSearch ? 'No courses found matching your search' : 'No courses available'}</p>
                </div>
              ) : (
                <div className="grid gap-2">
                  <p className="text-xs text-gray-500 mb-2">{filteredTemplateCourses.length} course{filteredTemplateCourses.length !== 1 ? 's' : ''} available</p>
                  {filteredTemplateCourses.map((course) => (
                    <button
                      key={course.id}
                      onClick={() => handleCreateFromTemplate(course)}
                      className="w-full p-4 text-left bg-gray-50 hover:bg-purple-50 border border-gray-200 hover:border-purple-300 rounded-xl transition-all group"
                    >
...
//         course_name: data.course_name,
//         course_description: data.course_description || undefined,
//         course_level: data.course_level || undefined,
//         course_number: data.course_number ? parseInt(data.course_number) : undefined,
//         course_alias: data.course_alias || undefined,
//         num_credits: data.num_credits ? parseFloat(data.num_credits) : undefined,
//       };
