...
//  * Verify setup token
//  */
// export async function verifySetupToken(token: string): Promise<{ valid: boolean; user?: { email: string; name: string }; error?: string }> {
//   return fetchAPI<{ valid: boolean; user?: { email: string; name: string }; error?: string }>(`/auth/verify-token/${token}`);
// }

// /**
//  * Setup password using token
//  */
// export async function setupPassword(token: string, password: string): Promise<LoginResponse> {
//   return fetchAPI<LoginResponse>('/auth/setup-password', {
//     method: 'POST',
...
//  * Generates new password and sends welcome email again
//  */
// export async function resendWelcomeEmail(userId: string): Promise<{ message: string }> {
//   return fetchAPI<{ message: string }>(`/users/${userId}/resend-welcome`, {
//     method: 'POST',
//   });
// }
...
/**
 * Bulk update grade_level for multiple grades
 */
export async function bulkUpdateGradeLevel(gradeIds: string[], gradeLevel: string): Promise<{ message: string }> {
  return fetchAPI<{ message: string }>('/grades/bulk-update-level', {
    method: 'PUT',
    body: JSON.stringify({ grade_ids: gradeIds, grade_level: gradeLevel }),
  });
}
